...
if (!query) {
	...
	console.log("\nExamples:");
	console.log(`  fnox exec -- ${scriptPath} \"javascript async await\"`);
	console.log(`  fnox exec -- ${scriptPath} \"rust programming\" -n 10`);
	console.log(`  fnox exec -- ${scriptPath} \"climate change\" --content`);
	console.log(`  fnox exec -- ${scriptPath} \"news today\" --freshness pd`);
	process.exit(1);
}
...
if (!apiKey) {
	...
	console.error(`If the key is stored in fnox, run: fnox exec -- ${scriptPath} \"query\"`);
	process.exit(1);
}
