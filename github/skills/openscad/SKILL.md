...
```bash
# Auto-detect model.json and use its first parameter set
./tools/export-stl.sh model.scad output.stl

# Pick a specific Customizer parameter set
./tools/export-stl.sh model.scad output.stl --customizer-preset "New set 1"

# Ignore the sidecar JSON and use only .scad defaults / -D overrides
...
# Explicitly choose / ignore a matching model.json sidecar
...
- **gridfinity** (`.pi/skills/gridfinity/`) — Gridfinity grid specs, library usage (`gridfinity_extended_openscad`), and parametric examples for baseplates and bins. Load this skill when designing gridfinity-compatible models. Requires OpenSCAD **developer snapshot** with `manifold` backend.
- **bambu-3mf** (`.pi/skills/bambu-3mf/`) — Create BambuStudio-compatible 3MF files from exported STLs and slice for printing.
